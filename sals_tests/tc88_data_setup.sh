...
postpaid_lkp_dir=input_data
postpaid_lkp_input=$postpaid_lkp_dir/$postpaid_lkp_file

recurring_lkp_file=tc8_input_data_recurring_lkp.txt
recurring_lkp_dir=input_data
recurring_lkp_input=$recurring_lkp_dir/$recurring_lkp_file
...
expected_output=$out_dir/tc88_result.expected
...
########################################################################################################################
########################################################################################################################
function setup_part1_data {

rm -f $throttle_input
touch $throttle_input
###################
# setup the EDR stream..
...
echo "now we move the paymenttype lkp into the cep engine.."
...
echo "now we move the recurring lkp into the cep engine.."
...
echo "now we move the EDRs into the cep engine.."
...
rm -f ${postpaid_lkp_dir}/2${postpaid_lkp_file}
echo "$msisdn1,$PaymentType1" >> ${postpaid_lkp_dir}/2${postpaid_lkp_file}
echo "$msisdn2,PREPAID"       >> ${postpaid_lkp_dir}/2${postpaid_lkp_file}
echo "$msisdn3,FONIC"         >> ${postpaid_lkp_dir}/2${postpaid_lkp_file}



echo "here is the postpaid lookup we used.."
cat ${postpaid_lkp_dir}/2${postpaid_lkp_file}
echo " "
echo "here is the EDR stream we used.. (this is the valid file)"
gzip -dc ${throttle_dir}/2${throttle_file}.gz

# we add the next set of results we expect
echo "I,N:$Time1,$msisdn1,$Quota_Name1,$Quota_Next_Reset_Time1,$TriggerType1,,,,,,,,,,,,,,,,,,,,,,,,$Quota_Status1,,,,$Quota_Usage1,,,,,,,,,,$PaymentType1,$(($Quota_Usage1 + $Quota_Usage1)),$IsRecurring1,$InitialVolume1" >> $expected_output

echo "now we move the paymenttype lkp into the cep engine.."
cp ${postpaid_lkp_dir}/2${postpaid_lkp_file} $data_dir/lookup_paymenttype/
touch $data_dir/lookup_paymenttype/2${postpaid_lkp_file}.done

#echo "sleeping 10 seconds.."
#sleep 10

echo "now we move the EDR stream into the cep engine.."
...
########################################################################################################################
########################################################################################################################
function setup_part3_data {

#we change values in recurring lkp..
InitialVolume2=5; IsRecurring2=N
echo ""
...
# we add the next set of results we expect
echo "I,N:$Time1,$msisdn1,$Quota_Name1,$Quota_Next_Reset_Time1,$TriggerType1,,,,,,,,,,,,,,,,,,,,,,,,$Quota_Status1,,,,$Quota_Usage1,,,,,,,,,,$PaymentType1,$(($Quota_Usage1 + $Quota_Usage1 + $Quota_Usage1)),$IsRecurring2,$InitialVolume2" >> $expected_output
echo "I,N:$Time1,$msisdn2,$Quota_Name1,$Quota_Next_Reset_Time1,$TriggerType1,,,,,,,,,,,,,,,,,,,,,,,,$Quota_Status1,,,,$Quota_Usage1,,,,,,,,,,$PaymentType1,$(($Quota_Usage1 + $Quota_Usage1)),$IsRecurring2,$InitialVolume2" >> $expected_output
echo "I,N:$Time1,$msisdn3,$Quota_Name1,$Quota_Next_Reset_Time1,$TriggerType1,,,,,,,,,,,,,,,,,,,,,,,,$Quota_Status1,,,,$Quota_Usage1,,,,,,,,,,$PaymentType1,$(($Quota_Usage1 + $Quota_Usage1)),$IsRecurring2,$InitialVolume2" >> $expected_output
echo "####################################################"
...
rm -f ${postpaid_lkp_dir}/3${postpaid_lkp_file}
echo "$msisdn1,$PaymentType1" >> ${postpaid_lkp_dir}/3${postpaid_lkp_file}
echo "$msisdn2,$PaymentType1" >> ${postpaid_lkp_dir}/3${postpaid_lkp_file}
echo "$msisdn3,$PaymentType1" >> ${postpaid_lkp_dir}/3${postpaid_lkp_file}


rm -f  ${recurring_lkp_dir}/3${recurring_lkp_file}

echo "$msisdn1,$Quota_Name1,$InitialVolume2,$IsRecurring2" >> ${recurring_lkp_dir}/3${recurring_lkp_file}
echo "$msisdn2,$Quota_Name1,$InitialVolume2,$IsRecurring2" >> ${recurring_lkp_dir}/3${recurring_lkp_file}
echo "$msisdn3,$Quota_Name1,$InitialVolume2,$IsRecurring2" >> ${recurring_lkp_dir}/3${recurring_lkp_file}


echo "now we move the paymenttype lkp into the cep engine.."
cp ${postpaid_lkp_dir}/3${postpaid_lkp_file} $data_dir/lookup_paymenttype/
touch $data_dir/lookup_paymenttype/3${postpaid_lkp_file}.done

#echo "sleeping 10 seconds.."
#sleep 10

echo "now we move the recurring lkp into the cep engine.."
cp ${recurring_lkp_dir}/3${recurring_lkp_file} $data_dir/lookup_recurring/
touch $data_dir/lookup_recurring/3${recurring_lkp_file}.done
...
#sleep 10
echo "here is the postpaid lookup we used.."
cat ${postpaid_lkp_dir}/3${postpaid_lkp_file}
echo " "
echo "here is the recurring lookup we used.."
cat ${recurring_lkp_dir}/3${recurring_lkp_file}
echo " "
echo "here is the EDR stream we used.. (this is the valid file)"
gzip -dc ${throttle_dir}/3${throttle_file}.gz
