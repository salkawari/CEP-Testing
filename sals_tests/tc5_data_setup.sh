...
throttle_file=tc5_EDR_UPCC231_MPU484_4924_40131211100031.csv
throttle_input=input_data/$throttle_file

prepaid_lkp_file=tc5_input_data_prepaid_lkp.txt
prepaid_lkp_input=input_data/$prepaid_lkp_file

recurring_lkp_file=tc5_input_data_recurring_lkp.txt
...
expected_output=$out_dir/tc5_result.expected
...
echo "Row9: a not valid prepaid throttle 100 event (Quota_Name = Q_110_roaming_Month)"
echo " "
echo ""
################################################################################
################################################################################
echo "TC5: 1. PCRD EDRs (input stream).."
rm -f $throttle_input
...
echo "$msisdn7,FONIC" >> $prepaid_lkp_input
echo "$msisdn8,$PaymentType1" >> $prepaid_lkp_input
echo "$msisdn9,POSTPAID" >> $prepaid_lkp_input
cp $prepaid_lkp_input $data_dir/lookup_paymenttype/
cp $prepaid_lkp_input $data_dir/lookup_paymenttype/${prepaid_lkp_file}.done

################################################################################
echo "TC5: 3. recurring lkp.."
...
################################################################################
echo "TC5: 4. generating the expected output.."
...
cat $data_dir/lookup_paymenttype/tc5_input_data_prepaid_lkp.txt
echo " "
echo "here is the recurring lookup we used.."
cat $data_dir/lookup_recurring/tc5_input_data_recurring_lkp.txt
echo " "
