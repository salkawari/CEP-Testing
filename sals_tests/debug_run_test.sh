...
then
  ...
  echo "missing parameters!! tc1.. and choice.."
  exit
fi
...
then
  ...
  rm -fr $data_dir
for i in $(echo "$esp_server_dir $data_dir $err_dir $jar_adapter_path $input_dir $data_dir/pcrf_files_prepaid $data_dir/pcrf_files_postpaid $data_dir/lookup_paymenttype $data_dir/lookup_paymenttype $data_dir/lookup_recurring $data_dir/output_prepaid $data_dir/output_postpaid $data_dir/output_fonic")
do
  if [ ! -d "$i" ]
  then
    mkdir -p $i
...
i=$1
  ...
  model_xml_def_path=$jar_adapter_path/EDR_PCRF_V6.36-${SINGLE_FLOW_TYPE}_load.xml
...
do
  ...
  echo "starting the stream viewer..."
  ./dfesp_streamviewer &
fi
