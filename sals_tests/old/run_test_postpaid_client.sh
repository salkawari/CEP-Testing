...
model_xml_postpaid=$jar_adapter_path/EDR_PCRF_V6.36-POSTPAID_client.xml
...
do
  ...
  ./dfesp_xml_server -pubsub 55555 -server 55556 -loglevel debug &>${my_loc}/server.txt &
  echo " "
  sleep 4

  echo "${test_case_name}: 6. starting the xml models dfesp_xml_client for $model_xml_postpaid.."
  ./dfesp_xml_client -server localhost:55556 -file $model_xml_postpaid &
  sleep 1



#  echo "${test_case_name}: 7. starting the POSTPAID_start.xml.."
...
#  ./dfesp_xml_client -server localhost:55556 -file POSTPAID_restore.xml &
  ...
  ################################################################################
  echo "${test_case_name}: 9. starting the adapters.."
  cd $jar_adapter_path
  ...
  echo "${test_case_name}:10. now we can compare results.."
...
#  echo "${test_case_name}: 11. starting the POSTPAID_persist.xml.."
#  cd $esp_server_dir  
#  ./dfesp_xml_client -server localhost:55556 -file POSTPAID_persist.xml &
#  
#  cd $my_loc
  ################################################################################
  echo "10. checking for processes to kill.."
