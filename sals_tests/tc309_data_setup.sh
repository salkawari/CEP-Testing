...
./requirement_13.sh
echo " "
echo "we make sure the join between the fonic throttle 100 event works correctly"
echo "(via msisdn and  the QuotaName). We also confirm that when no lookup entry is "
echo "found, the fonic throttle 100 event reaches the output but with these 2 "
echo "attributes empty. E.g. fonic throttle 100 events shouldnt be seen in the output of the CEP Engine."
echo " "
echo " "
echo "EDR data setup:"
echo "Row1: a ${SINGLE_FLOW_TYPE} throttle 100, Quota_name=Q_110_local_Month"
echo "Row2: a ${SINGLE_FLOW_TYPE} throttle 100, Quota_name=Q_23_local_Month"
echo " "
echo " "
