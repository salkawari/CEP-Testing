function remove_persist() {
for i in $(echo persist_fonic_throttle_event persist_prepaid_throttle_event persist_postpaid_throttle_event persist_postpaid_2nd_step)
do
  rm -fr $i
  mkdir $i
done
...
echo "############################"
echo "defining output stream.."
echo "$DFESP_HOME/bin/dfesp_fs_adapter -Q -k sub -h 'dfESP://localhost:55555/POSTPAID_SECOND_STEP/SECOND_STEP_EVENT_QUERY/PCRF_DATA_USAGE_STREAM' -b 1 -t csv -f pcrf.csv"
$DFESP_HOME/bin/dfesp_fs_adapter  -k sub -h "dfESP://localhost:55555/POSTPAID_SECOND_STEP/SECOND_STEP_EVENT_QUERY/PCRF_DATA_USAGE_STREAM" -b 1 -t csv -f pcrf.csv
echo "sleep 1"
sleep 1
echo "############################"
