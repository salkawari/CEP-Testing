...
echo "to confirm only valid combinations result in a processed postpaid throttle 80 event"
echo " "
echo "EDR data setup:"
echo "Row1: a valid ${SINGLE_FLOW_TYPE} throttle 80 event (Quota_Name=Q_110_local_Month, TriggerType=2, Quota_Status=2, QuotaValue=1)"
...
echo "Row6: a not valid ${SINGLE_FLOW_TYPE} throttle 100 event (as its PREPAID)"
...
Quota_Status1=2;                                               g_Quota_Status=$Quota_Status1; #p3
...
echo "$msisdn6,PREPAID" >> $paymenttype_lkp_input
