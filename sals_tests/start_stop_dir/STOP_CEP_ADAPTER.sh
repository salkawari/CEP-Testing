...
already_running_process=$(ps aux|grep -i jar| grep -v grep | awk '{print $2}')
...
while [ $(ps aux|grep -i jar| grep -v grep | awk '{print $2}'|wc -w) -ne 0 ]
do
  echo "waiting on the following processes to stop.."
  echo $(ps aux|grep -i jar| grep -v grep | awk '{print $2}')
  sleep 3
done
