...
recurring_lkp_file=tc6_input_data_recurring_lkp.txt
...
expected_output=$out_dir/tc6_result.expected
...
################################################################################
################################################################################
echo "tc6: 1. PCRD EDRs (input stream).."
rm -f $throttle_input
...
cat $data_dir/lookup_paymenttype/tc6_input_data_postpaid_lkp.txt
echo " "
echo "here is the recurring lookup we used.."
cat $data_dir/lookup_recurring/tc6_input_data_recurring_lkp.txt
echo " "
